...
pub struct InventoryContractData {
    metadata: InventoryContractMetadata,
    owner_id: AccountId,
}

fn save_state(contract: &InventoryContract) {
    /* 
...
fn load_state(filename: &str) -> InventoryContract {
    /* 
    contract.nft_approve( {
    let filename_tokens = "tokens.json";
...
fn init_contract(args: &Args, filename: Option<String>) -> InventoryContract {
    /* 
    contract.nft_approve( {
...
fn test_mint(args: Args, contract: &mut InventoryContract) {
    /* 
    contract.nft_approve() {
...
fn test_transfer(args: Args, contract: &mut InventoryContract) {
    /*
    contract.nft_approve() {
     ...
     */
    let new_token = contract.asset_token(args.token_id.to_string());
    println!("{}", serde_json::to_string(&new_token).unwrap());
}

fn test_add_asset_license(args: Args, contract: &mut InventoryContract) {
    /*
    contract.nft_approve(
        args.token_id.to_string(),
...
fn test_remove_asset_license(args: Args, contract: &mut InventoryContract) {
    /*
    contract.nft_approve(
        args.token_id.to_string(),
        AccountId::new_unchecked(args.account_id),
...
fn test_update_asset_license(args: Args, contract: &mut InventoryContract) {
    /*
    contract.nft_approve(
        args.token_id.to_string(),
        AccountId::new_unchecked(args.account_id),
...
fn main() {
    ...
    let mut contract: InventoryContract;
    if args.action == "new" {
        contract = init_contract(&args, None);
        println!("{}", serde_json::to_string(&contract.inventory_metadata()).unwrap());
    } else {
